...
def calc_periodic_kernel(x_QF, x_train_NF=None, length_scale=1.0, period=1.0):
    ...
    for f in range(F):
        xq_f_Q1 = x_QF[:,f].reshape((Q,1))
        xtr_f_1N = x_train_NF[:,f].reshape((1,N))
        dist_QN = xq_f_Q1 - xtr_f_1N
        square_sin_QN = np.square(np.sin(np.pi / period * dist_QN))
        k_QN = np.exp(- 0.5 * square_sin_QN / np.square(length_scale))
